...
_find8DirRoute(map, x, y, isEnemyNoMine) {
    ...
    for (let i = 0, len = dirArray.length; i < len; i++) {
        ...
        if(isEnemyNoMine && absNextId === 12){
            routeArray.push( this.toHexString(tempX, tempY));
            continue;
        }
        ...
        if(absId === 1) {//工兵
            if (absNextId === 11 || absNextId === 1 || absNextId === 0) {
                routeArray.push(this.toHexString(tempX, tempY));//军棋暂时不放
            }
            continue;
        }
    ...
    if (this._isXRailRoad(x, y)) {
        ...
        for (let down = y - 1; down > 0; down--) {
            if (this._isRoute(id, map[x][down], isEnemyNoMine)) {
                routeArray.push(this.toHexString(x , down));
            }
            break;
        }
        //向上走动
        for (let up = y + 1; up < 11; up++) {
            if (this._isRoute(id, map[x][up], isEnemyNoMine)) {
                routeArray.push(this.toHexString(x , up));
            }
        }
    }
    ...
    if (this._isYRailRoad(y)) {
        ...
        for (let left = x - 1; left >= 0; left--) {
            if (this._isRoute(id, map[left][y], isEnemyNoMine)) {
                routeArray.push(this.toHexString(left , y));
            }
        }
        //向右走动
        for (let right = x + 1; right < 5; right++) {
            if (this._isRoute(id, map[right][y], isEnemyNoMine)) {
                routeArray.push(this.toHexString(right , y));
            }
        }
    }
...
_findGongBingRoute(map, x,y, isEnemyNoMine){
    ...
    for(let i=0; i<  5; i++){
        for(let j= 0; j< 12; j++){
            if( tempMap[i][j] === 1){
                routeArray.push(this.toHexString(i, j));
            }
        }
    }
    return routeArray;
...
toHexString(x, y){
    return new Number(x).toString(16) + new Number(y).toString(16);
}
}
let map = [];
for (let x = 0; x < 5; x++) { //x轴， 棋盘左下角为原点
...
for(let aa of a1){
    let aa0 = parseInt(aa[0], 16);
    let aa1 = parseInt(aa[1], 16);
    console.info("aa0=%d, aa1=%d", aa0, aa1);
}