    ...
    _findComanRoute(map, x, y, isEnemyNoMine) {
        ...
        if (this._isXRailRoad(x, y)) {
            ...
            for (let down = y - 1; down > 0; down--) {
                bRoute = this._isRoute(id, map[x][down], isEnemyNoMine);
                if (bRoute === 1) {
                    routeArray.push([x, down]);
                    continue;//碰到空的就继续，否则就break
                } else if (bRoute === 2) {
                    routeArray.push([x, down]);
                }
                break;
            ...
            for (let up = y + 1; up < 11; up++) {
                bRoute = this._isRoute(id, map[x][up], isEnemyNoMine);
                if (bRoute === 1) {
        ...
        if (this._isYRailRoad(y)) {
            ...
            for (let left = x - 1; left >= 0; left--) {
                bRoute = this._isRoute(id, map[left][y], isEnemyNoMine);
                if (bRoute === 1) {
            ...
            for (let right = x + 1; right < 5; right++) {
                bRoute = this._isRoute(id, map[right][y], isEnemyNoMine);
                if (bRoute === 1) {
