const scorePoint  = require("./evaluate-point.js");
const zobrist = require( "./zobrist.js");
...
class Board {
  ...
  init (sizeOrBoard) {
    ...
    var size = 15;
    if(sizeOrBoard instanceof Array && sizeOrBoard.length) {
    ...
    // 存储双方得分
    this.comScore = statistic.create(size, size)
    this.humScore = statistic.create(size, size)

    // scoreCache[role][dir][row][column]
    this.scoreCache = [
      [], // placeholder
      [ // for role 1
        statistic.create(size, size),
        statistic.create(size, size),
        statistic.create(size, size),
        statistic.create(size, size)
      ],
