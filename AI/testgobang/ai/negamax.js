 ...
 */
const math = require('./math'); //数学函数
const config = require('./config'); //配置
const board = require('./board'); //棋盘
const statistic = require('./statistic'); //静态
...
var deeping = function(candidates, role, deep) {
  ...
  //candidates = newCandidates.length ? newCandidates : [candidates[0]] // 必败了，随便走走
    ...
    // if (math.littleThan(bestScore, SCORE.THREE * 2)) bestScore = MIN // 如果能找到双三以上的棋，则保留bestScore做剪枝，否则直接设置为最小值
  }
...
//  return role === R.com ? (p.scoreCom >= SCORE.TWO) : (p.scoreHum >= SCORE.TWO)
//})
//const defendPoints = candidates.filter((p) => {
//  return role === R.hum ? (p.scoreCom >= SCORE.TWO) : (p.scoreHum >= SCORE.TWO)
//})
//let attack, defend
...
//let result
////进攻优先，只要对面不能成活三，就不防守
//if (!defend || defend.min < SCORE.THREE) result = attack
//else if (!attack) result = defend
//// 如果双方都有可以赢的点
//// 注意处理冲四和活四的关系，活四分比冲四高，但是注意他的优先级和活四是一样的
//else {
//  if (attack.min > SCORE.BLOCKED_FOUR / 2 && attack.min < SCORE.BLOCKED_FOUR *2) attack.min = SCORE.FOUR
//  if (defend.min > SCORE.BLOCKED_FOUR / 2 && defend.min < SCORE.BLOCKED_FOUR *2) defend.min = SCORE.FOUR
